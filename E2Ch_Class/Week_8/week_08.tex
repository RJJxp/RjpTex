...
% 对于上次课程, `thin client interface', 我想了稍许. 我们经常用的软件本质上就是把一些程序的接口的实现放在了按钮点击动作上, `client interface' 用户使用的接口, 接口一般只是软件开发维护者使用, 用户根本不可能触及到, 所以可以直接将该词语译为 `应用'. 班级同学对该词语查找到有类似 `细用户界面' 或是 `粗用户界面' 的翻译,  我觉得这种翻译很奇怪. 

% 我猜想之所以会有如此的 `名词'. 是因为这个的译者没有太多计算机方面的经验, 自己不会也不去问专业人士, 赶到项目节点, 搞出一个朗朗上口的 `瘦用户界面', 颇有自导自演, 皇帝新装的意味. 看来对于专业领域的翻译, 在翻译初期, 单打独斗是不行的.
